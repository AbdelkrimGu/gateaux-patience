// Render consistent, on-brand top-down box thumbnails (square / heart / oval)
// from the real cocoa-tiramisu surface (base/cacao.png), so the picker thumbnail
// matches exactly what the customizer previews. Flat-lay look: soft shadow,
// cream rim (the tiramisu's cream side), warm background.
//
// Run: node scripts/render-boxes.mjs   →   public/images/tiramisu/boxes/box-*.png
...
const BG = 800;                 // output canvas
const SIZE = 560;               // cocoa shape size

// Shape path generators, normalised to an s×s box.
const SHAPES = {
...
const bg = Buffer.from(
  `<svg xmlns="http://www.w3.org/2000/svg" width="${BG}" height="${BG}"><defs><radialGradient id="g" cx="50%" cy="40%" r="68%"><stop offset="0%" stop-color="#FFFDFB"/><stop offset="100%" stop-color="#F1E4D6"/></radialGradient></defs><rect width="100%" height="100%" fill="url(#g)"/></svg>`
);
...
for (const [name, fn] of Object.entries(SHAPES)) {
  ...
  const shaped = await sharp(cocoa)
    .composite([{ input: mask, blend: "dest-in" }])
    .png()
    .toBuffer();

  const rimSize = SIZE + 20;
  const rim = await sharp(svg(fn(rimSize), rimSize, "#ece0cf")).png().toBuffer();

  const shSize = SIZE + 8;
  const shadow = await sharp(svg(fn(shSize), shSize, "#3a2414", 0.3))
    .blur(17)
    .png()
    .toBuffer();
  ...
  await sharp(bg)
    .composite([
      { input: shadow, left: center(shSize), top: center(shSize) + 20 },
      { input: rim, left: center(rimSize), top: center(rimSize) },
      { input: shaped, left: center(SIZE), top: center(SIZE) },
    ])
    .png()
    .toFile(join(OUT, `box-${name}.png`));
  console.log(`box-${name}.png`);
}
