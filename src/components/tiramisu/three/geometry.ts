...
function makeUVGenerator(bb: ShapeBBox) {
  // Straight silhouette→[0,1]² map. The top cap now only carries the bare cocoa
  // photo (letters of both styles are real geometry), so no mirror correction is
  // needed here — the base is effectively symmetric.
